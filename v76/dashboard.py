...
st.set_page_config(page_title="V7.6 Fusion", layout="wide")
st.title("Polymarket V7.6 Live Signal Fusion")

con = sqlite3.connect("portfolio.db")
df = pd.read_sql_query("select * from snapshots order by id", con)

tabs = st.tabs(["Live Markets","Top Movers","Signals","Paper Trades","Analytics"])
...
with tabs[1]:
    ...
    for m in df["market"].unique():
        ...
        if len(s)==2:
            ...
            rows.append({"market":m,"prev_yes":round(prev,4),"curr_yes":round(curr,4),"change_pct":round(ch,2)})
...
with tabs[2]:
    ...
    if 'mv' in locals() and not mv.empty:
        for _,r in mv.iterrows():
            ...
            sig.append({"market":r["market"],"signal":action,"reason":reason,"move_pct":r["change_pct"]})
    st.dataframe(pd.DataFrame(sig), use_container_width=True)

with tabs[3]:
    st.write("Paper trades module ready for next upgrade.")

with tabs[4]:
    st.metric("Snapshots", len(df))
    if not df.empty:
        st.line_chart(df["yes_price"])
